...
protected:
  // Helper to prepend ./ to relative paths to prevent option injection
  // where filenames starting with '-' are treated as flags by ImageMagick
  std::string safeFilePath(const std::string &path) {
    if (path.empty() || path[0] == '/' ||
        (path.length() >= 2 && path[0] == '.' && path[1] == '/')) {
      return path; // Already absolute or explicitly relative
    }
    return "./" + path;
  }
