...
protected:
  ...
  // Helper to safely format a file path for command line usage (prevents option injection)
  std::string safeFilePath(const std::string &path) {
    if (path.empty()) return path;
    // Prepend ./ if the path doesn't start with / or ./ to prevent it being parsed as a flag
    if (path[0] != '/' && (path.length() < 2 || path.substr(0, 2) != "./")) {
        return "./" + path;
    }
    return path;
  }
