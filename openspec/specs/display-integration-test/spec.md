...
### Requirement: Cross-Test State Isolation
The application MUST reset its internal state during setup to ensure that subsequent test runs are not affected by data from previous runs.

#### Scenario: Isolated Test Execution
- **GIVEN** a test suite running multiple integration tests in sequence
- **WHEN** the `App_Setup` function is called at the start of each test
- **THEN** it internally calls `App_ResetState`
- **AND** all historical data, timers, and state variables are cleared
- **AND** tests pass reliably regardless of execution order
