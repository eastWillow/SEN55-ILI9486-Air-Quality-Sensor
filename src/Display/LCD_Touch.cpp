...
unsigned char TP_Scan(unsigned char chCoordType)
{
    ...
    if (pressed) {
        ...
        // Also update sTP_DEV.Xpoint/Ypoint just in case, though they are ADC values in real HW
        sTP_DEV.Xpoint = x;
        sTP_DEV.Ypoint = y;
...
// Stubs for internal functions that are not needed for PC but referenced by TP_Adjust
...
{
    if(x) *x = sTP_Draw.Xpoint;
    if(y) *y = sTP_Draw.Ypoint;
}
