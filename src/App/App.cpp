...
static void DrawButton(const Button &btn, bool inverted) {
  if (inverted) {
    ...
    GUI_DrawRectangle(btn.x, btn.y, btn.x + btn.w, btn.y + btn.h, BLUE,
                      DRAW_FULL, DOT_PIXEL_DFT);
    GUI_DisString_EN(btn.x + 10, btn.y + 8, btn.label, &Font16, BLUE,
                     LCD_BACKGROUND);
  } else {
    ...
    GUI_DisString_EN(btn.x + 10, btn.y + 8, btn.label, &Font16, LCD_BACKGROUND,
                     BLUE);
  }
}
